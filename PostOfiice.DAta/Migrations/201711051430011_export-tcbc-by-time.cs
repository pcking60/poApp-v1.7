namespace PostOfiice.DAta.Migrations
{
    ...
    public partial class exporttcbcbytime : DbMigration
    {
                ...
                (
                    ...
                    (
                        ...
                        where
				            ...
				            and ts.Status=1 and ts.IsCash=1 and sg.MainServiceGroupId=3 and (sg.ID=93 or sg.ID=75) and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date)
                        group by s.Name, ps.[Percent]
    	            ) st
    	            full outer join
    	            (
    		            select
    			            s.Name, sum(td.Money) as Money, ps.Name as PName, s.VAT, ps.[Percent]
    	                from ServiceGroups sg
                            ...
                            inner join Districts d
                            on p.DistrictID = d.ID
                            where ps.Name like N'Sản lượng%' and ts.Status=1 and sg.MainServiceGroupId=3 and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date)
                    ...
                    (
			            ...
			            from
                            ...
                            where
					            (ps.Name like N'Số tiền%' or ps.Name like N'Phí%')
					            and ps.Name not like N'%cước%'
					            and ts.Status=1 and ts.IsCash=1 and sg.MainServiceGroupId=3 and sg.ID=94 and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date)
                            group by s.Name, ps.[Percent]
                    ) st1
    	            on sl.Name = st1.name
		            full outer join
		            ...
		            on sl.Name = st3.Name
    	            group by sl.Name, sl.Money, st.Money, st1.Money, sl.VAT, st1.[Percent], st.[Percent], sl.[Percent], st3.Money, st3.[Percent]) rs
            group by rs.ServiceName, rs.Quantity, rs.VAT");
        }

        public override void Down()
        {
            DropStoredProcedure("Export_TCBC_BY_TIME");
        }
    }
}
namespace PostOfiice.DAta.Migrations
{
    ...
    public partial class group_by_service_district_po_time_bccp : DbMigration
    {
        public override void Up()
        {
            CreateStoredProcedure(
                   "Export_By_Service_Group_And_Time_District_Po_BCCP",
                   p => new
                   {
                       fromDate = p.String(),
                       toDate = p.String(),
                   ...
                   @"select 
				    ...
				    from 
	                    ...
	                    where (ps.Name like N'Số tiền%' or ps.Name like N'Phí%') and ts.Status=1 and ts.IsCash=0 and sg.MainServiceGroupId=1 and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date) and d.ID=@districtId and p.ID=@poId
	                ...
	                full outer join 
	                    ...
	                    inner join Districts d
	                    on p.DistrictID = d.ID
	                    where ps.Name like N'Sản lượng%' and ts.Status=1 and sg.MainServiceGroupId=1 and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date) and d.ID=@districtId and p.ID=@poId
	                ...
	                full outer join
	                    ...
	                    inner join Districts d
	                    on p.DistrictID = d.ID
	                    where (ps.Name like N'Số tiền%' or ps.Name like N'Phí%') and ts.Status=1 and ts.IsCash=1 and sg.MainServiceGroupId=1 and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date) and d.ID=@districtId and p.ID=@poId
	                    group by s.Name, ps.[Percent]
	                    ) st1
	                on sl.Name = st1.name
        ...
        {
            DropStoredProcedure("Export_By_Service_Group_And_Time_District_Po_BCCP");
        }
    }
}

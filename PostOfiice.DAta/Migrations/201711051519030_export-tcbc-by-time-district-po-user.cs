namespace PostOfiice.DAta.Migrations
{
    ...
    public partial class exporttcbcbytimedistrictpouser : DbMigration
    {
        public override void Up()
        {
            CreateStoredProcedure(
                "Export_TCBC_BY_TIME_DISTRICT_PO_USER",
                p => new
                {
                    fromDate = p.String(),
                    toDate = p.String(),
    ...
    inner join ApplicationUsers u
    	                    ...
    	                    where (ps.Name like N'Số tiền%' or ps.Name like N'Phí%')
								...
								and ts.Status=1 and ts.IsCash=1 and sg.MainServiceGroupId=3 and (sg.ID=93 or sg.ID=75) and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date) and d.ID=@districtId and p.ID=@poId and u.Id=@userId
    	                ...
    	                full outer join
    	                    ...
    	                    inner join Districts d
    	                    on p.DistrictID = d.ID
    	                    where ps.Name like N'Sản lượng%' and ts.Status=1 and sg.MainServiceGroupId=3 and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date) and d.ID=@districtId and p.ID=@poId and u.Id=@userId
    ...
    inner join ApplicationUsers u
    	                    ...
    	                    where (ps.Name like N'Số tiền%' or ps.Name like N'Phí%')
								and ps.Name not like N'%cước'
								and ts.Status=1 and ts.IsCash=1 and sg.MainServiceGroupId=3 and sg.ID=94 and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date) and d.ID=@districtId and p.ID=@poId and u.Id=@userId
    	                    group by s.Name, ps.[Percent]
    	                    ) st1
    	                on sl.Name = st1.name
						full outer join
    ...
    inner join ApplicationUsers u
    	                    ...
    	                    inner join Districts d
    	                    on p.DistrictID = d.ID
    	                    where (ps.Name like N'%cước%') and ts.Status=1 and ts.IsCash=1 and sg.MainServiceGroupId=3 and (sg.ID=93 or sg.ID=75) and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date) and d.ID=@districtId and p.ID=@poId and u.Id=@userId
    	                    group by s.Name, ps.[Percent]
    	                    ) st3
							on sl.Name = st3.Name
    	            group by sl.Name, sl.Money, st.Money, st1.Money, sl.VAT, st1.[Percent], st.[Percent], sl.[Percent], st3.Name, st3.Money, st3.[Percent]) rs
    group by rs.ServiceName, rs.Quantity, rs.VAT");
        }

        public override void Down()
        {
            DropStoredProcedure("Export_TCBC_BY_TIME_DISTRICT_PO_USER");
        }
    }
}
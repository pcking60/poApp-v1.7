namespace PostOfiice.DAta.Migrations
{
    ...
    inner join ApplicationUsers u
    	                    ...
    	                    where (ps.Name like N'Số tiền%' or ps.Name like N'Phí%') and ts.Status=1 and ts.IsCash=1 and sg.MainServiceGroupId=3 and (sg.ID=93 or sg.ID=75) and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date) and d.ID=@districtId 
    ...
    inner join ApplicationUsers u
    	                    ...
    	                    inner join Districts d
    	                    on p.DistrictID = d.ID
    	                    where (ps.Name like N'Số tiền%' or ps.Name like N'Phí%') and ts.Status=1 and ts.IsCash=1 and sg.MainServiceGroupId=3 and sg.ID=94 and CAST( ts.TransactionDate as date) between CAST(@fromDate as date) and CAST(@toDate as date) and d.ID=@districtId 
    	                    group by s.Name, ps.[Percent]
    	                    ) st1
    	                on sl.Name = st1.name
